...
- 가변인수 메서드를 호출하면 인수의 개수와 길이가 같은 배열을 만들고, 인수들을 만들어진 배열에 저장한 후에 가변인수 메서드에 전달
...
- 가변인수가 포함된 메서드가 호출될 때마다 **배열을 새로 할당하고 초기화하기 때문**
<br>
- 아래와 같은 패턴으로 변경할 수도 있음
...
- 위 메서드 호출의 95%가 3개 이하의 인수를 사용한다고 가정
- 그렇기 때문에 가변인수는 5%의 호출을 담당 
        - (특별한 경우에 유용하게 사용된다고 함, EnumSet의 정적 팩토리?도 이 기법을 사용해 열거 타입 집합 생성 비용을 최소화함)
...
- 때론, 빈 컨테이너를 할당할 때에도 비용이 들어, null을 반환하는 쪽이 낫다는 주장이 있지만 두가지 면에서 틀린 주장임
...
public Cheese[] getCheeses() {
    ...
    // toArray는 원소가 하나라도 있다면 배열을 새로 생성함
...
#### **활용3. 항상 값이 채워짐을 가정 (NoSuchElementException을 주의)**
...
- Optional<List<T>>를 반환하는 것보다 그저 빈 리스트 List<T>를 반환하는 것이 좋고,
- 빈 컨테이너를 그대로 반환하면 클라이언트에 옵셔널 처리 코드를 만들지 않아도 된다

<br>

**옵셔널을 맵의 값으로 사용하면 절대 안됨**

- Map에 사용하는 것을 예로, 맵 안에 키가 없다는 정의가 2가지 있는데,
        1. “키 자체가 없는 경우”와 
        2. “키는 있지만 속이 빈 옵셔널인 경우”, 이 2가지로 모호해지며 오류 가능성이 커짐
